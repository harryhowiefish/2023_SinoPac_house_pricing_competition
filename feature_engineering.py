...
def load_data(path: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    ext_trading = ext_trading.iloc[ext_trading[[
        '單價元平方公尺', '橫坐標', '縱坐標']].dropna().index]
...
def zoning_correction(df: pd.DataFrame,
                      reference_df: pd.DataFrame,
                      r: int = 100) -> pd.DataFrame:
    for row in tqdm(df.iloc):
        data = reference_df[(reference_df['鄉鎮市區'] == row['鄉鎮市區'])]
        data = data[(data['橫坐標'].between(row['橫坐標']-r, row['橫坐標']+r))
                    & (data['縱坐標'].between(row['縱坐標']-r, row['縱坐標']+r))]
        try:
            if data['都市土地使用分區'].value_counts().index[0] in [
                    '住', '其他', '商', '農', '工']:
                df.loc[row.name, '使用分區'] = data['都市土地使用分區'
...
def location_k_cluster(train, test, ext_trading, cluster=500):
    ...
    train = pd.concat([train, pd.DataFrame(kmeans.transform(train_data),
                                           columns=[
                      f"k{i}" for i in range(1, cluster+1)])], axis=1)
    test_data = test[['橫坐標', '縱坐標']].to_numpy()
    test = pd.concat([test, pd.DataFrame(kmeans.transform(test_data), columns=[
                     f"k{i}" for i in range(1, cluster+1)])], axis=1)
    ...
    ext_trading['cluster'] = kmeans.predict(
        ext_trading[['橫坐標', '縱坐標']].to_numpy())

    for table in [train, test]:
        map = all_data.groupby('cluster')['ID'].count().to_dict()
        table["cluster_based_internal_sales"] = table['cluster'].map(map)
        map = all_data.groupby('cluster')['主要用途'].value_counts(
            normalize=True).to_dict()
        table["cluster_based_purpose_ratio"] = table.apply(
            lambda x: map[(x['cluster'], x['主要用途'])], axis=1)
        map = all_data.groupby('cluster')['建物型態'].value_counts(
            normalize=True).to_dict()
        table["cluster_based_build_type_ratio"] = table.apply(
            lambda x: map[(x['cluster'], x['建物型態'])], axis=1)
...
def feature_k_cluster(train, test):
    all_data = pd.concat([train, test], axis=0)
    for name, cluster_num, cluster_col in zip(
            ['car', 'size', 'floor'],
            [20, 50, 30],
            [['車位面積', '車位個數'],
             ['主建物面積', '土地面積', '陽台面積', '附屬建物面積'],
             ['移轉層次', '總樓層數']]):
        ...
        train = pd.concat([train,
                           pd.DataFrame(
                               kmeans.transform(train[cluster_col].to_numpy()),
                               columns=[f"{name}_k{i}" for i in range(1, cluster_num+1)])], axis=1)  # noqa
        test = pd.concat([test,
                          pd.DataFrame(
                              kmeans.transform(test[cluster_col].to_numpy()),
                              columns=[f"{name}_k{i}" for i in range(1, cluster_num+1)])], axis=1)  # noqa
        train[f'{name}_cluster'] = kmeans.predict(
            train[cluster_col].to_numpy())
...
def add_ext_trading(table, ext_trading):
    ...
    for idx, row in tqdm(enumerate(table.iloc)):
        ...
        for r in radius_options[::-1]:
            temp = temp[(temp['橫坐標'].between(x-r, x+r)) &
                        (temp['縱坐標'].between(y-r, y+r))]
...
def add_ext_location(path, table):
    ...
    for index, row in tqdm(enumerate(table[['橫坐標', '縱坐標']].to_numpy())):
        for ext_type in external_types:
            table.loc[index, f"dist_to_{ext_type}"] = cdist(
                [row],
                ext_loc[ext_loc['Name'] == ext_type][['lng', 'lat']].to_numpy()).min()  # noqa
        [x, y] = row
        temp = ext_loc.copy()
        for r in radius_options[::-1]:
            temp = temp[temp['lng'].between(
                x-r, x+r) & temp['lat'].between(y-r, y+r)]
...
def add_open_data(folder_path, table):
    ...
    pop_density = pd.read_csv(
        f'{folder_path}/pop_density_processed.csv', index_col=0)
    result = []
...
def perfect_matching(table, ext_trading, dist=100):
    for row in tqdm(table.iloc):
        ...
        data = data[(data['建物型態'] == row['建物型態']) &
                    (data['主要建材'] == row['主要建材'])]
        data = data[(data['總樓層數'] == row['總樓層數']) & (
            data['移轉層次'].between(row['移轉層次']-2, row['移轉層次']+2))]
        data['dist'] = cdist(data[['橫坐標', '縱坐標']].to_numpy(), [
                             row[['橫坐標', '縱坐標']].to_list()])
...
def external_data_target_encoding(train, test, ext_trading, cluster=50):
    ...
    ext_trading['cluster_50'] = kmeans.predict(
        ext_trading[['橫坐標', '縱坐標']].to_numpy())

    # External trading info
    # neighbor area sales count and median_price
    radius_options = [200, 1000, 2000]
    median_map = ext_trading.groupby(
        'cluster_50')['單價元平方公尺'].median().to_dict()
    min_map = ext_trading.groupby('cluster_50')['單價元平方公尺'].min().to_dict()

    for table in [train, test]:
        table['cluster_50_external_median_price'] = table['cluster_50'].map(
            median_map)
        table['cluster_50_external_minprice'] = table['cluster_50'].map(
            min_map)
        median_price = []
        road_med_price = []
        for idx, row in tqdm(enumerate(table.iloc)):
            ...
            temp = ext_trading[(ext_trading['建物型態'] == row['建物型態']) & (
                ext_trading['鄉鎮市區'] == row['鄉鎮市區'])]
            price_sub_list = []
            for r in radius_options[::-1]:
                temp = temp[(temp['橫坐標'].between(x-r, x+r)) &
                            (temp['縱坐標'].between(y-r, y+r))]
...
def make_all_feature(path: str) -> None:
    ...
    for table in [train, test]:
        ...
        table = add_ext_location(
            f'{path}/external_data/external_transformed_simplified.csv', table)
