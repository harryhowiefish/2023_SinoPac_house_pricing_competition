...
class mapeMetric(object):
    ...
    def evaluate(self, approxes, target, weight):
        ...
        for i in range(len(approx)):
            ...
            weight_sum += w
            error_sum += w * \
                abs((np.e**approx[i] - np.e**target[i])/np.e**target[i])

        return error_sum, weight_sum


def training(model: cb.core.CatBoostRegressor):
    ...
    model.select_features(train_dataset, eval_set=val_dataset, steps=5,
                          features_for_select=train_dataset.get_feature_names(),  # noqa
                          num_features_to_select=422)
    return model
...
def main():
    model = cb.CatBoostRegressor(loss_function="RMSE",
                                 ...
                                 rsm=0.5, learning_rate=0.01, l2_leaf_reg=2,
                                 random_strength=5, eval_metric=mapeMetric(),
                                 random_seed=5)
